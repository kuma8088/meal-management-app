...
日本食品標準成分表とOpen Food Factsのデータを取得し、用途に応じて保存：
- **日本食品標準成分表（2,538件）** → DynamoDB（高速検索・キャッシング用）
- **Open Food Facts（1.1GB）** → S3（大規模データ、AI検索参照用）
...
- [x] DynamoDB インポート実行（AWS環境で実行）- **成功: 2,538件全て正常インポート**
- [x] インポート結果確認 - **テーブル名修正とDecimal型対応**
...
### 1. マルチレベルヘッダーの抽出
- **課題**: Excelファイルが複雑なマルチレベルヘッダー構造
  - Row 2-7に分散したヘッダー情報
  - Row 3に実際のカラム名（食品名、栄養値）が存在
- **解決方法**:
  - 列1-3: Row 2から取得（食品群、食品番号、索引番号）
  - 列4以降: Row 3から取得（食品名、栄養値など）
  - フォールバック: Row 3に値がない場合はRow 2を参照

### 2. DynamoDB データ型の互換性
- **課題**: DynamoDBが浮動小数点数（Float）を直接サポートしない
- **解決方法**: Pythonの `Decimal` 型を使用
  - すべての栄養値をDecimal型で保存
  - データベース互換性を確保

### 3. データ値のクリーニング
- **課題**: 推定値が括弧で囲まれている（例：「(11.3)」）
- **解決方法**: 括弧を削除してから変換
  ```python
...
### 4. テーブル名の修正
- **課題**: スクリプトのテーブル名「Foods」が実際のAWSテーブル名と不一致
- **解決方法**: 正しいテーブル名「meal-management-app-foods-dev」に更新
...
**作成日**: 2025-12-06
**最終更新**: 2025-12-06
**ステータス**: ✅ 完了 - Phase 1（日本食品標準成分表）のDynamoDBインポート完了
...
|------|------|
| **インポート日時** | 2025-12-06 |
| **食品マスタデータ件数** | 2,538件（100%成功） |
| **テーブル名** | meal-management-app-foods-dev |
| **ソース** | japanese_standard（日本食品標準成分表）|
| **栄養情報** | カロリー、タンパク質、脂質、炭水化物 |
| **サンプル食品** | 魚類 > あなご 蒸し（カロリー:723kcal） |
